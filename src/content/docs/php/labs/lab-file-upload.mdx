...
```php
// TODO: Create a GET route for '/upload' that maps to UploadController::class 'index' method
//       Set the route name to 'upload.index'

// TODO: Create a POST route for '/upload' that maps to UploadController::class 'upload' method
//       Set the route name to 'upload.process'
```
...
2. Visit `http://localhost/[your-app-folder-name]/upload` in your browser
...
| ---------------- | ------ | -------- | --------------------- | -------------------------------------------- |
| `directory`      | string | Yes      | Upload directory path | APP_BASE_DIR_PATH . '/public/uploads/images'` |
...
$result = FileUploadHelper::upload($uploadedFile, [
    'directory' => APP_BASE_DIR_PATH . '/public/uploads',
...
The general idea is to have a helper class that can be used to upload files to a specific directory. The helper class will validate the file, check the file size, check the file type, and generate a unique filename. The helper class will then move the file to the specific directory.

**Code Reusability:**
```php
// Use the same helper for profile pictures
$result = FileUploadHelper::upload($file, [
    'directory' => App_BASE_DIR_PATH . '/public/uploads/avatars',
    'allowedTypes' => ['image/jpeg', 'image/png'],
    'maxSize' => 1 * 1024 * 1024,
    'filenamePrefix' => 'avatar_'
]);
...
$result = FileUploadHelper::upload($file, [
    'directory' => APP_BASE_DIR_PATH . '/public/uploads/docs',
