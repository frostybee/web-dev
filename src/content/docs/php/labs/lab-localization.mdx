...
class LocaleMiddleware implements MiddlewareInterface
{
    ...
    public function process(Request $request, RequestHandler $handler): ResponseInterface
    {
        ...
        // Priority 2: Session value (use SessionManager::get('locale'))
        // Priority 3: Default locale
        // Hint: Use multiple ?? operators to chain fallbacks
        // Example: $selectedLocale = $urlLocale ?? SessionManager::get('locale') ?? $this->translator->getDefaultLocale();
        ...
        // Use: SessionManager::set('locale', $locale) to save it
        ...
        // Get session locale using: SessionManager::get('locale')
...
2. If URL parameter provided and valid:
   ...
   - **Save it to session** using `SessionManager::set('locale', $locale)`
3. Else if session has a saved locale and it's valid:
   - Set it in translator
4. Store current locale in request attribute
5. Continue to next middleware

**Important Notes:**
- SessionManager uses **static methods** (`SessionManager::get()` and `SessionManager::set()`)
...
2. **LocaleMiddleware** runs second and can then use SessionManager to get/set the locale

**What happens if the order is wrong?**
- If LocaleMiddleware runs before SessionMiddleware, the session won't be started yet
- Calling `SessionManager::get('locale')` would fail or return incorrect values
...
**Possible Causes:**
- Forgot to call `SessionManager::set('locale', $locale)` in LocaleMiddleware
- SessionMiddleware not registered or running after LocaleMiddleware
- Session not started
- Incorrect middleware order

**Solution:**
1. Check that `process()` method in LocaleMiddleware saves locale to session using `SessionManager::set('locale', $locale)` when URL parameter is provided
2. Ensure SessionMiddleware is registered BEFORE LocaleMiddleware in `config/middleware.php`:
   ...
   ```
3. Verify SessionMiddleware is calling `SessionManager::start()`
4. Check browser cookies are enabled (use browser dev tools → Application → Cookies)
5. Test by checking session directly: Add `var_dump(SessionManager::get('locale'));` in your view
