...
4. Create a view file `views/demo/counter.php`:
...
|--------|-------------|---------|
| `set($key, $value)` | Store data in the session | `SessionManager::set('user_id', 123)` |
| `get($key, $default)` | Retrieve data (with optional default) | `SessionManager::get('user_id', null)` |
| `has($key)` | Check if key exists | `if (SessionManager::has('user_id'))` |
| `remove($key)` | Remove specific key | `SessionManager::remove('cart_items')` |
| `clear()` | Clear all data (keep session alive) | `SessionManager::clear()` |
| `destroy()` | Destroy session completely | `SessionManager::destroy()` |
