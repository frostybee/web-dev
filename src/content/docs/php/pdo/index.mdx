...
function getUsers($pdo, $page = 1, $limit = 20) {
    $offset = ($page - 1) * $limit;
...
$stmt->execute();
$count = $stmt->fetchColumn(); // For single values

$stmt = $pdo->prepare("SELECT username FROM users");
$stmt->execute();
$usernames = $stmt->fetchAll(PDO::FETCH_COLUMN); // For single column arrays
?>
```
...
### 1. Use a Database Abstraction Layer
...
class UserRepository {
    ...
    public function findById($id) {
        $sql = "SELECT id, username, email, first_name, last_name, created_at FROM users WHERE id = :id";
        $stmt = $this->pdo->prepare($sql);
        $stmt->execute([':id' => $id]);
        return $stmt->fetch();
    }
...
PDO provides a robust, secure, and flexible way to interact with MySQL databases in PHP. By following these patterns and best practices, you can build scalable and maintainable database-driven applications.
