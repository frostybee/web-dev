...
Open your a terminal in VS Code and run the following commands:
...
class TwoFactorController extends BaseController
{
    ...
    public function __construct(
        ...
        TwoFactorAuthModel $twoFactorModel,
        UserModel $userModel
    ) {
    ...
    public function disable(Request $request, Response $response): Response
    {
        ...
        $validUser = $this->userModel->verifyPassword($user['email'], $password);
...
class TwoFactorMiddleware implements MiddlewareInterface
{
    ...
    public function __construct(TwoFactorAuthModel $twoFactorModel)
    {
        $this->twoFactorModel = $twoFactorModel;
    }
...
> **Note:** This follows the same container registration pattern from previous labs.
...
// Add TwoFactorAuthModel
TwoFactorAuthModel::class => function (ContainerInterface $c) {
    return new TwoFactorAuthModel($c->get(PDOService::class));
},

// Add TwoFactorMiddleware (inject TwoFactorAuthModel)
TwoFactorMiddleware::class => function (ContainerInterface $c) {
    return new TwoFactorMiddleware($c->get(TwoFactorAuthModel::class));
},

// Add TwoFactorController (inject TwoFactorAuthModel and UserModel)
...
Update `AuthController::Authenticate()` - after setting session data (around line 152), update to:
...
## Step 11: Create TrustedDeviceModel
...
// Update the class properties and constructor:
private TwoFactorAuthModel $twoFactorModel;
private UserModel $userModel;
...
public function __construct(
    ...
    TwoFactorAuthModel $twoFactorModel,
    UserModel $userModel,
    TrustedDeviceModel $trustedDeviceModel
) {
    parent::__construct($container);
    $this->twoFactorModel = $twoFactorModel;
    $this->userModel = $userModel;
    $this->trustedDeviceModel = $trustedDeviceModel;
}
```
...
## Step 14: Update Container Registrations for Device Trust

**Objective:** Update the dependency injection container to include TrustedDeviceModel.

Update `config/container.php` with the following changes:
...
// Update TwoFactorMiddleware to inject TrustedDevice
TwoFactorMiddleware::class => function (ContainerInterface $c) {
...
// Update TwoFactorController to inject TrustedDevice
TwoFactorController::class => function (ContainerInterface $c) {
    return new TwoFactorController(
        $c,
        $c->get(TwoFactorAuthModel::class),
