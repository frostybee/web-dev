...
<FullscreenModal buttonText="🚀 Open the interactive HTTP request-response cycle simulator" modalTitle="Interactive HTTP Request-Response Cycle">
...
**Safe & Idempotent:**
- Safe: Only retrieves data, doesn't modify anything on the server.
- Idempotent: Calling it multiple times produces the same result (no side effects). 
...
- Not safe: Modifies server state (creates/updates resources).
- Not idempotent: Each request can create a new resource or have different effects.
- Example: Submitting a form that creates a new order—each POST creates another order.
