...
echo "Highest Math grade: $highestMath\n";
```
...
foreach ($byDepartment as $dept => $deptEmployees) {
    ...
    echo "$dept average salary: $" . number_format($avgSalary) . "\n";
}
```
...
PHP arrays are versatile data structures that can store multiple values efficiently. Whether you're working with simple indexed arrays, complex associative arrays, or multidimensional structures, understanding the various syntaxes and built-in functions will help you write more effective PHP code.
